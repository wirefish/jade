...
(defentity stevedore (humanoid)
  ...
  (:when-talk ((actor &quest jade.isle-of-dawn:special-delivery done) self topic)
    (tell self actor "Hey, thanks. I've been needin' those. Here's your coin!
       ...
       town.")
    (give self actor (list (clone-entity 'silver-coin :quantity 10)))
...
def plazaPortal: lib.continuingPortal {
    brief = "the plaza"
}

def plazaLocation: location {
    name = "Bayside Plaza"
...
deflocation baysidePlaza1: plazaLocation {
    contents = [beggar]
    exits = [plazaPortal -> 'east to baysidePlaza2]
}

deflocation baysidePlaza2: plazaLocation {
    exits = [plazaPortal -> 'west to baysidePlaza1,
             plazaPortal -> 'east to baysidePlaza3,
             ruttedPath -> 'north to westDock]
}

deflocation baysidePlaza3: plazaLocation {
    exits = [plazaPortal -> 'west to baysidePlaza2, plazaPortal -> 'east to baysidePlaza4,
             dirtRoad -> 'south to harborRoad1]
...
def youngLaborer: lib.npc {
    ...
    offersQuests = [danceMonkey]

    when talk(actor: .quest(danceMonkey, available), self, topic) {
        ...
        offerQuest(self, danceMonkey, actor)
    }

    after acceptQuest(actor, quest: danceMonkey, self) {
        tell(self, actor) |
    ...
    when talk(actor: .quest(danceMonkey, active), self, topic) {
        tell(self, actor) |
            Did you find the monkey? When you're done I won't make you do it all
            again, I promise.
    }

    when talk(actor: .quest(danceMonkey, done), self, topic) {
        tell(self, actor) |
...
deflocation baysidePlaza4: plazaLocation {
    contents = [youngLaborer]
    exits = [plazaPortal -> 'west to baysidePlaza3, plazaPortal -> 'east to baysidePlaza5,
             ruttedPath -> 'north to eastDock]
}

defquest ticketPlease {
    name = "Ticket, Please"
...
def ticketDispenser: fixture {
    ...
    when use(actor: .quest(ticketPlease, active), self, target) {
        receiveItems(actor, [yellowTicket], self)
        showTutorial(actor, 'giveItems) |
        ...
        advanceQuest(actor, ticketPlease)
    }
...
def attendant: lib.npc {
    ...
    offersQuests = [ticketPlease]

    when talk(actor: .quest(ticketPlease, available), self, topic) {
        tell(self, actor) |
            You look like one of those reborn heroes who keep popping up. I bet
            you need some coin. I'd be happy to be of service!
        offerQuest(self, ticketPlease, actor)
    }

    after acceptQuest(actor, quest: ticketPlease, self) {
        tell(self, actor) |
    ...
    when talk(actor: .quest(ticketPlease, active), self, topic) {
        tell(self, actor) |
            Go ahead and `use` the dispenser. It won't bite.
    }

    allow giveItem(actor, item: yellowTicket, self) {
        return true
    }

    after giveItem(actor: .quest(ticketPlease, done), item: yellowTicket, self) {
        tell(self, actor) |
        ...
        receiveItems(actor, [50 of lib.silverCoin], self)
        completeQuest(actor, ticketPlease, 'done)
    }
}

deflocation baysidePlaza5: plazaLocation {
    contents = [attendant, ticketDispenser]
    exits = [plazaPortal -> 'west to baysidePlaza4]
}

// harbor road

def harborRoad: location {
    name = "Harbor Road"
    description = |
...
deflocation harborRoad1: harborRoad {
    exits = [dirtRoad -> 'north to baysidePlaza3, dirtRoad -> 'south to harborRoad2,
             lib.entryDoorway -> 'east to minersGuildhall]
}
...
def miningTrainer: lib.miningTrainer {
    brief = "Marigold"
...
def miningVendor: lib.npc {  // FIXME: lib.vendor
    ...
    sells = [lib.copperPickaxe, lib.bronzePickaxe]
...
def squarePortal: lib.continuingPortal {
  brief = "the square"
}

def shaggyDog: lib.npc {
...
deflocation squareNw: square {
    ...
    exits = [squarePortal -> 'east to squareN, squarePortal -> 'south to squareW,
             lib.entryDoorway -> 'north to armoryTrainingHall,
             lib.entryDoorway -> 'west to swordShop]
}
...
deflocation squareNe: square {
    exits = [squarePortal -> 'west to squareN, squarePortal -> 'south to squareE,
             lib.entryDoorway -> 'east to innCommonRoom,
             lib.exitDoorway -> 'north to axeShop]
}
...
deflocation squareW: square {
    exits = [squarePortal -> 'north to squareNw, squarePortal -> 'east to squareC,
             ...
             lib.entryDoorway -> 'west to towerLibrary]
}

def squareSign: fixture {
...
deflocation squareSw: square {
    exits = [squarePortal -> 'north to squareW, squarePortal -> 'east to squareS,
             dirtRoad -> 'west to forestRoad1, lib.entryDoorway -> 'south to lodgeFoyer]
}
...
deflocation innCommonRoom: innRoom {
    ...
    exits = [lib.exitDoorway -> 'west to squareNe,
             lib.doorway -> 'north to innKitchen,
             lib.stairway -> 'up to innUpstairsHall]
}

def cookingTrainer: lib.npc {
...
deflocation innUpstairsHall: innRoom {
    ...
    exits = [lib.stairway -> 'down to innCommonRoom,
             lib.doorway -> 'west to smallGuestRoom,
             lib.doorway -> 'east to largeGuestRoom]
}
...
def warriorTrainer: lib.npc {  // FIXME: lib.warriorTrainer
    brief = "Andalya"
...
deflocation lodgeFoyer: lodgeRoom {
    ...
    exits = [lib.exitDoorway -> 'north to squareSw,
             lib.doorway -> 'south to lodgeTrophyRoom,
             lib.doorway -> 'west to lodgeStudy]
}

def stuffedGorilla: fixture {
...
deflocation lodgeTrophyRoom: lodgeRoom {
    ...
    exits = [lib.doorway -> 'north to lodgeFoyer, lib.doorway -> 'west to lodgeWorkshop]
}

def lordOlmquist: lib.npc {
    brief = "Lord Olmquist"
...
deflocation lodgeStudy: lodgeRoom {
    ...
    exits = [lib.doorway -> 'east to lodgeFoyer, lib.doorway -> 'south to lodgeWorkshop]
}

def explorerTrainer: lib.npc {
    brief = "Naman Artani"
...
deflocation lodgeWorkshop: lodgeRoom {
    ...
    exits = [lib.doorway -> 'north to lodgeStudy, lib.doorway -> 'east to lodgeTrophyRoom,
             lib.exitDoorway -> 'south to wallStreet1]
}

// mages' tower

def towerRoom: location {
    domain = 'indoor
    surface = 'stone
...
deflocation towerLibrary: towerRoom {
    ...
    exits = [lib.exitDoorway -> 'east to squareW,
             lib.stairway -> 'down to towerBasement,
             lib.stairway -> 'up to towerStudy]
}

def wandVendor: lib.npc {
    brief = "Giglox Tiglox"
...
deflocation eastRoad1: eastRoad {
    exits = [dirtRoad -> 'west to squareE, dirtRoad -> 'east to eastRoad2,
             lib.entryDoorway -> 'south to maceShop]
}
...
deflocation eastRoad3: eastRoad {
    exits = [dirtRoad -> 'west to eastRoad2, dirtRoad -> 'east to eastGate,
             lib.entryDoorway -> 'north to templeSanctuary]
}

// mace shop

def maceVendor: lib.npc {
    ...
    sells = [lib.copperMace, lib.bronzeMace, lib.copperMaul, lib.bronzeMaul]
...
deflocation potionShop: location {
    ...
    exits = [lib.exitDoorway -> 'west to muggersAlley1,
             lib.doorway -> 'east to alchemyWorkshop]
}

def venomExtractor: item {
...
def alchemist: lib.npc {
    ...
    when talk(actor: .quest(getSomVenom, done), self, topic) {
        tell(self, actor) |
            ...
            reward.
        receiveItems(actor, [20 of lib.silverCoin], self)
    }
}
...
def daggerVendor: lib.npc {
    ...
    sells = [lib.copperDagger, lib.bronzeDagger]
...
deflocation warehouseAnteroom: warehouse {
    ...
    exits = [lib.exitDoorway -> 'west to muggersAlley3,
             lib.doorway -> 'east to warehouseStoreroom,
             lib.stairway -> 'up to warehouseOffice]
}

def vagabondTrainer: lib.npc {
    brief = "Shady Roger"
...
deflocation warehouseStoreroom: warehouse {
    ...
    exits = [lib.doorway -> 'west to warehouseAnteroom,
             lib.doorway -> 'east to warehouseMeetingRoom]
}
...
deflocation southRoad1: southRoad {
    exits = [dirtRoad -> 'north to squareS, dirtRoad -> 'south to southRoad2,
             lib.entryDoorway -> 'east to lootedShop]
}

deflocation southRoad2: southRoad {
    exits = [dirtRoad -> 'north to southRoad1, dirtRoad -> 'south to wallStreet3,
             lib.entryDoorway -> 'east to spearShop]
}

// looted shop

def distraughtShopkeep: lib.npc {
    brief = "Mirabel"
...
deflocation wallStreet1: wallStreet {
    ...
    exits = [cobbledRoad -> 'east to wallStreet2,
             lib.entryDoorway -> 'north to lodgeWorkshop,
             lib.entryDoorway -> 'west to frontDesk]
}
...
deflocation wallStreet4: wallStreet {
    exits = [cobbledRoad -> 'west to wallStreet3, cobbledRoad -> 'east to wallStreet5,
             lib.entryDoorway -> 'south to locksmithShop]
}
...
deflocation wallStreet6: wallStreet {
    exits = [cobbledRoad -> 'west to wallStreet5, alley -> 'north to muggersAlley3,
             lib.entryDoorway -> 'east to craftersHall1nw]
}

// armor emporium

def armorEmporium: location {
    subregion = "Armor Emporium"
    domain = 'indoor
    surface = 'wood
}

def armorGreeter: lib.npc {
...
deflocation frontDesk: armorEmporium {
    ...
    exits = [lib.exitDoorway -> 'east to wallStreet1,
             lib.doorway -> 'north to mediumArmorShop,
             lib.doorway -> 'west to lightArmorShop]
}

def leatherArmorVendor: lib.npc {
    brief = "Lingum"
    description = |
...
deflocation lightArmorShop: armorEmporium {
    ...
    exits = [lib.doorway -> 'east to frontDesk, lib.doorway -> 'north to heavyArmorShop]
...
deflocation mediumArmorShop: armorEmporium {
    ...
    exits = [lib.doorway -> 'south to frontDesk, lib.doorway -> 'west to heavyArmorShop]
...
deflocation heavyArmorShop: armorEmporium {
    ...
    exits = [lib.doorway -> 'south to lightArmorShop, lib.doorway -> 'east to mediumArmorShop]
...
def evend: gangMember {
    brief = "Evend"
    weapons = []  // FIXME: lib.sword
    questKey = 'evend
...
def goon: gangMember {
    ...
    weapons = []  // FIXME: lib.mace
}

def ruffian: gangMember {
    brief = "a lanky ruffian"
    weapons = []  // FIXME: lib.dagger
}
